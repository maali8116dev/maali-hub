import { useNavigate } from "react-router-dom";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Check, Zap, Users, ArrowRight } from "lucide-react";

const FREE_FEATURES = [
  "Browse all opportunities",
  "Access public resources & guides",
  "Join the MAALI community network",
  "Monthly newsletter",
];

const MEMBER_FEATURES = [
  "Everything in Community",
  "Apply to opportunities",
  // "Priority application review",
  // "Direct messaging with reviewers",
  // "Featured profile in the network",
  "Exclusive member events & workshops",
];

const MembershipSection = () => {
  const navigate = useNavigate();

  return (
    <section className="py-20 bg-muted/30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="text-center mb-14">
          <Badge variant="outline" className="mb-4 text-primary border-primary/30 bg-primary/5">
            Membership
          </Badge>
          <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-4">
            Join the MAALI Network
          </h2>
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            Connect with Africa's most active funding ecosystem. Browse for free, or unlock the
            full platform for less than a cup of coffee a month.
          </p>
        </div>

        {/* Tier cards */}
        <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {/* Community ΓÇö Free */}
          <Card className="flex flex-col border-2 border-border">
            <CardContent className="flex flex-col h-full p-8">
              <div className="flex items-center gap-2 mb-2">
                <Users className="h-5 w-5 text-muted-foreground" />
                <span className="font-semibold text-foreground">Community</span>
              </div>
              <div className="text-4xl font-bold text-foreground mb-1">Free</div>
              <p className="text-sm text-muted-foreground mb-6">No credit card required</p>

              <ul className="space-y-3 mb-8 flex-1">
                {FREE_FEATURES.map((f) => (
                  <li key={f} className="flex items-start gap-2 text-sm text-foreground">
                    <Check className="h-4 w-4 text-success mt-0.5 shrink-0" />
                    {f}
                  </li>
                ))}
              </ul>

              <Button
                variant="outline"
                className="w-full"
                onClick={() => navigate("/join")}
              >
                Join Free
              </Button>
            </CardContent>
          </Card>

          {/* Full Member $2/month */}
          <Card className="flex flex-col border-2 border-primary relative shadow-elegant">
            <div className="absolute -top-3 left-1/2 -translate-x-1/2">
              <Badge className="bg-primary text-primary-foreground px-4 py-1">Most Popular</Badge>
            </div>
            <CardContent className="flex flex-col h-full p-8">
              <div className="flex items-center gap-2 mb-2">
                <Zap className="h-5 w-5 text-primary" />
                <span className="font-semibold text-foreground">Full Member</span>
              </div>
              <div className="flex items-end gap-1 mb-1">
                <span className="text-4xl font-bold text-foreground">$2</span>
                <span className="text-muted-foreground mb-1">/ month</span>
              </div>
              <p className="text-sm text-muted-foreground mb-6">Cancel anytime. No lock-in.</p>

              <ul className="space-y-3 mb-8 flex-1">
                {MEMBER_FEATURES.map((f) => (
                  <li key={f} className="flex items-start gap-2 text-sm text-foreground">
                    <Check className="h-4 w-4 text-primary mt-0.5 shrink-0" />
                    {f}
                  </li>
                ))}
              </ul>

              <Button
                variant="hero"
                className="w-full group"
                onClick={() => navigate("/join")}
              >
                Become a Member
                <ArrowRight className="ml-2 h-4 w-4 transition-transform group-hover:translate-x-1" />
              </Button>
            </CardContent>
          </Card>
        </div>

        {/* Social proof */}
        <p className="text-center text-sm text-muted-foreground mt-10">
          Join thousands of African entrepreneurs already building with MAALI.
        </p>
      </div>
    </section>
  );
};

export default MembershipSection;
