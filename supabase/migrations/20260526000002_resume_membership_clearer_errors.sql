-- Replace resume_membership with version that returns specific error per failure case.
...
BEGIN
  ...
  IF v_tier <> 'member' OR v_status <> 'active' THEN
    RAISE EXCEPTION 'Membership is not active Full Member (tier=%, status=%). Re-subscribe instead of resume.', v_tier, v_status;
  END IF;
  ...
  RETURN jsonb_build_object('stripe_subscription_id', v_stripe_subscription_id);
END;
$$;
