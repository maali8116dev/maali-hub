-- Extend opportunity search to match translated title/description (fr, pt, de).

CREATE OR REPLACE FUNCTION public.get_opportunities_with_filters(
  p_opportunity_type text DEFAULT NULL::text,
  p_program_format text DEFAULT NULL::text,
  p_funding_type text DEFAULT NULL::text,
...
BEGIN
  ...
  WHERE (p_opportunity_type IS NULL OR o.opportunity_type::text = p_opportunity_type)
    ...
    AND (v_search IS NULL OR (
      o.title ILIKE '%' || v_search || '%'
      OR o.description ILIKE '%' || v_search || '%'
      OR p.name ILIKE '%' || v_search || '%'
      OR coalesce(o.translations->'fr'->>'title', '') ILIKE '%' || v_search || '%'
      OR coalesce(o.translations->'fr'->>'description', '') ILIKE '%' || v_search || '%'
      OR coalesce(o.translations->'pt'->>'title', '') ILIKE '%' || v_search || '%'
      OR coalesce(o.translations->'pt'->>'description', '') ILIKE '%' || v_search || '%'
      OR coalesce(o.translations->'de'->>'title', '') ILIKE '%' || v_search || '%'
      OR coalesce(o.translations->'de'->>'description', '') ILIKE '%' || v_search || '%'
    ))
